...
def union_polygons(Layers):
    for layer, lay_data in Layers.iteritems():
        if (layer == 'JJ') or (layer == 'JP') or (layer == 'JC'):
            utils.tools.union_wire(Layers, layer, 'result')
        else:
            utils.tools.union_wire(Layers, layer, 'jj')
            utils.tools.union_wire(Layers, layer, 'result')
...
class Process:
    ...
    def calculate_sub_atom(self, atom, subatom):
        ...
        if subj and clip:
            result_list = utils.tools.angusj(clip, subj, subatom['method'])
            if result_list:
